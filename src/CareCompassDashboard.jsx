...
const SAGE_KEYFRAMES = `
  ...
  @keyframes ffFlyIn {
    0%   { top: -80px; left: 50vw; transform: translate(-50%,0) scale(1.4); opacity: 0; }
    8%   { opacity: 1; }
    30%  { top: 22vh; left: 65vw; transform: translate(-50%,0) scale(1.3) rotate(-12deg); }
    55%  { top: 45vh; left: 25vw; transform: translate(-50%,0) scale(1.2) rotate(8deg); }
    75%  { top: 70vh; left: 75vw; transform: translate(-50%,0) scale(1.1) rotate(-6deg); }
    90%  { top: calc(100vh - 120px); left: calc(100vw - 80px); transform: translate(-50%,0) scale(0.85); }
    100% { top: calc(100vh - 100px); left: calc(100vw - 58px); transform: translate(-50%,0) scale(1); opacity: 1; }
  }
  @keyframes ffLand {
...
const FireflyMark = ({ size = 36, style: extraStyle = {} }) => (
  <svg width={size} height={size} viewBox="0 0 72 72" fill="none" xmlns="http://www.w3.org/2000/svg"
    style={{ animation: "ffDrift 4s ease-in-out infinite", display: "block", ...extraStyle }}>
    <circle cx="36" cy="36" r="34" fill="#e8f0eb" stroke="#7a9e87" strokeWidth="1"/>
...
const DASHBOARD_SYSTEM_PROMPT = `You are Sage, the friendly Care Compass guide embedded in the user's personal dashboard. Care Compass helps people with chronic illness track symptoms, prepare for appointments, understand patterns, and navigate their health journey.

You can help the user understand their dashboard, explain features like the symptom tracker, appointment prep, AI insights, and doctor reports. You can also offer encouragement and general guidance about using Care Compass effectively.

Do not provide medical advice or interpret their specific symptoms. If they share health concerns, validate them warmly and encourage them to use the full Care Compass tools or speak with their doctor.

Keep responses warm, supportive, and concise — 2-4 sentences. No bullet points or markdown.`;
...
const DASHBOARD_SUGGESTIONS = [
  ...
  "What's a doctor report?",
  "How do I log symptoms?",
];

function SageChatbot() {
  // fly-in states: "flying" | "landing" | "landed"
  const [flyState, setFlyState] = useState("flying");
  ...
  // Fly-in sequence: fly for 2.2s, land bounce for 0.4s, then show FAB + greeting
  useEffect(() => {
    const flyTimer = setTimeout(() => setFlyState("landing"), 2200);
    const landTimer = setTimeout(() => {
      setFlyState("landed");
      setTimeout(() => setGreeting(true), 800);
    }, 2600);
    return () => { clearTimeout(flyTimer); clearTimeout(landTimer); };
  }, []);
    ...
    <>
      ...
      {flyState === "flying" && (
        <div style={{
          ...
          animation: "ffFlyIn 2.2s cubic-bezier(0.25, 0.46, 0.45, 0.94) forwards",
        }}>
          <FireflyMark size={80} />
        </div>
      )}
      ...
      {flyState === "landing" && (
        <div style={{
          position: "fixed", bottom: "1.5rem", right: "1.5rem", zIndex: 9999, pointerEvents: "none",
          animation: "ffLand 0.4s cubic-bezier(0.34,1.56,0.64,1) forwards",
        }}>
          <FireflyMark size={68} />
        </div>
      )}
      ...
      {/* ── Greeting bubble ── */}
      {flyState === "landed" && greeting && !open && (
        <div style={ss.greeting}>
          <p style={ss.greetingText}>Hi, I'm Sage! I'm here to help you navigate your dashboard. 🌿</p>
      ...
      {open && (
        <div style={ss.drawer}>
          ...
          <div style={ss.messages}>
            {messages.length === 0 && (
              <div>
                <div style={ss.emptyState}>How can I help you today?</div>
      ...
      {/* ── FAB (shown after landing) ── */}
      {flyState === "landed" && !open && (
