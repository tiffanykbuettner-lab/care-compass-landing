...
function inferType(name) {
  ...
  if (n.includes("cardio") || n.includes("optional")) return "optional";
  return "standard";
}

function inferDuration(type) {
  if (type === "flare") return 15;
  if (type === "rest")  return 0;
  if (type === "optional") return 30;
  return 45;
}

function recommendedSessionName(checkin) {
  if (!checkin || checkin.feel === "rest") return null;
  if (checkin.feel === "flare" || (checkin.feel === "okay" && checkin.energy === "low")) {
    const hasUpper = checkin.joints?.some(j => ["Shoulders", "Neck / ribs"].includes(j));
    return hasUpper ? "Flare Session A" : "Flare Session B";
  }
  const dow = new Date().getDay();
  return (dow === 3 || dow === 4) ? "Full Body B" : "Full Body A";
}
...
function WeekDot({ code, day, type, isToday, isDone, isFlare }) {
  const bg     = isDone ? SAGE_DARK : isFlare ? AMBER : isToday ? "#fff" : type === "rest" ? CREAM : AMBER_LIGHT;
  ...
  return (
    <div style={{ background: "#fff", borderRadius: "1rem", border: "1px solid rgba(0,0,0,0.07)", borderLeft: `4px solid ${typeColor}`, padding: "1rem 1.25rem", display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "1rem" }}>
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <p style={{ fontSize: "0.78rem", color: TEAL, margin: "0 0 0.2rem", fontWeight: 500 }}>{formatDate(session.date)} · {session.duration} min</p>
...
function LogSessionModal({ initial = {}, onSave, onCancel }) {
  ...
  const [form, setForm] = useState({
    ...
    type:        defaultType,
    date:        todayStr(),
    duration:    initial.duration ?? inferDuration(defaultType),
    painLevel:   "",
    energyLevel: "",
    notes:       "",
  });
  ...
  function handleNameChange(e) {
    ...
    setForm(f => ({ ...f, sessionName: name, type, duration: inferDuration(type) }));
  }
    ...
    <div style={styles.modalOverlay}>
      <div style={styles.modalCard}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.25rem" }}>
          <h2 style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: "1.2rem", fontWeight: 700, color: INK, margin: 0 }}>
            {isFlare ? "Log a flare session" : "Log a session"}
          </h2>
        ...
        {isFlare && (
          <div style={{ background: AMBER_LIGHT, border: `1px solid ${AMBER}`, borderRadius: "0.75rem", padding: "0.75rem 1rem", marginBottom: "1rem", fontSize: "0.82rem", color: AMBER_DARK, lineHeight: 1.6 }}>
            Flare day. The goal today isn't progress — it's maintaining the habit without worsening symptoms. That still counts.
          </div>
        )}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
          <div>
            ...
            <select value={form.sessionName} onChange={handleNameChange} style={{ ...inp, WebkitAppearance: "none" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.75rem" }}>
            ...
            <div>
              ...
              <input type="number" value={form.duration} onChange={set("duration")} min="0" max="180" style={inp}/>
            </div>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.75rem" }}>
            <div>
          ...
          <div>
            ...
            <textarea value={form.notes} onChange={set("notes")} rows={3}
              placeholder={isFlare ? "What's flaring? Any modifications needed?" : "How did it feel? Any modifications? Joint notes?"}
              style={{ ...inp, resize: "vertical", lineHeight: 1.6 }}/>
          </div>
          <div style={{ display: "flex", gap: "0.75rem", marginTop: "0.25rem" }}>
            <button onClick={onCancel} style={styles.btnSecondary}>Cancel</button>
            <button
              onClick={() => { if (form.sessionName && form.date) onSave({ ...form, id: Date.now(), duration: Number(form.duration) || 0 }); }}
...
function CheckInModal({ onSave, onCancel }) {
  ...
  const rec = (() => {
    if (!feel) return null;
    if (feel === "rest") return { label: "Full rest day", sessionName: null, color: WARM_GRAY, bg: CREAM, border: SAGE };
    if (feel === "flare" || (feel === "okay" && energy === "low")) {
      const hasUpper = joints.some(j => ["Shoulders", "Neck / ribs"].includes(j));
      const name = hasUpper ? "Flare Session A" : "Flare Session B";
      return { label: `${name} — shoulders / ribs`, sessionName: name, color: AMBER_DARK, bg: AMBER_LIGHT, border: AMBER };
    }
    const dow = new Date().getDay();
    const name = (dow === 3 || dow === 4) ? "Full Body B" : "Full Body A";
    return { label: `${name} — Strength + Stability`, sessionName: name, color: SAGE_DARK, bg: SAGE_LIGHT, border: SAGE };
  })();
    ...
    <div style={styles.modalOverlay}>
      <div style={{ ...styles.modalCard, maxWidth: 520 }}>
        ...
        <div style={{ display: "flex", gap: "0.75rem" }}>
          ...
          <button
            onClick={() => { if (feel) onSave({ date: todayStr(), feel, joints, energy, recommendation: rec?.label, sessionName: rec?.sessionName }); }}
            disabled={!feel}
            style={{ ...styles.btnPrimary, background: feel === "flare" ? AMBER_DARK : SAGE_DARK, opacity: !feel ? 0.4 : 1 }}
          >
            {feel === "rest" ? "Mark as rest day" : "Log this session"}
          </button>
        </div>
      </div>
    </div>
...
export default function MovementCompassDashboard() {
  ...
  /* ── modal helpers ── */
  function openLogModal(initial = {}) {
    setLogInitial(initial);
    setShowLogModal(true);
  }

  function openTodaySession() {
    const name = todayCheckin?.sessionName || recommendedSessionName(todayCheckin) || "Full Body A";
    openLogModal({ sessionName: name, type: inferType(name) });
  }

  function openFlareSession() {
    openLogModal({ sessionName: "Flare Session B", type: "flare", duration: 15 });
  }
  ...
  /* check-in → auto-open log modal pre-filled with recommended session */
  function handleCheckinSave(checkin) {
    setTodayCheckin(checkin);
    saveCheckin(checkin);
    setShowCheckin(false);
    if (checkin.feel !== "rest" && checkin.sessionName) {
      openLogModal({ sessionName: checkin.sessionName, type: inferType(checkin.sessionName) });
    }
  }

  /* quick action definitions — all wired to real actions */
  const QUICK_ACTIONS = [
    {
      label:  "Start today's session",
      desc:   todayCheckin?.sessionName || recommendedSessionName(todayCheckin) || "Full Body A — strength + stability",
      color:  AMBER_DARK,
      action: openTodaySession,
    },
    {
      label:  "Log a flare day",
      desc:   "Switch to minimum-dose mode",
      color:  TEAL,
      action: openFlareSession,
    },
    {
      label:  "Exercise library",
      desc:   "Browse modifications by joint",
      color:  SAGE_DARK,
      action: () => { window.location.href = "/movement/library"; },
    },
    {
      label:  "View progress",
      desc:   "Strength gains + consistency",
      color:  INK_LIGHT,
      action: () => sessionsRef.current?.scrollIntoView({ behavior: "smooth", block: "start" }),
    },
  ];
  ...
  return (
    <div style={styles.root}>
          ...
          <div style={styles.navLinks}>
            {NAV.map(n => (
              <button key={n.key} onClick={() => setActiveNav(n.key)}
                style={{ ...styles.navLink, ...(activeNav === n.key ? styles.navLinkActive : {}) }}>
                {n.label}
              </button>
            ))}
          </div>
      ...
      <main style={styles.main}>
        <div style={styles.container}>
          ...
          <div style={styles.header}>
            <div>
              ...
              <p style={styles.subtitle}>
                {todayCheckin
                  ? `Today: ${todayCheckin.recommendation || "session ready"} · ${formatDate(todayStr())}`
            ...
            <div style={{ background: todayCheckin.feel === "flare" ? AMBER_LIGHT : todayCheckin.feel === "rest" ? CREAM : SAGE_LIGHT, border: `1px solid ${todayCheckin.feel === "flare" ? AMBER : todayCheckin.feel === "rest" ? "rgba(0,0,0,0.08)" : SAGE}`, borderRadius: "1rem", padding: "1rem 1.5rem", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "0.75rem" }}>
              <div>
                ...
                {todayCheckin.joints?.length > 0 && (
                  <span style={{ fontSize: "0.75rem", color: WARM_GRAY, display: "block", marginTop: 2 }}>Flagged: {todayCheckin.joints.join(", ")}</span>
                )}
              </div>
              <button onClick={openTodaySession} style={{ ...styles.btnPrimary, background: todayCheckin.feel === "flare" ? AMBER_DARK : SAGE_DARK, fontSize: "0.85rem", padding: "0.6rem 1.25rem" }}>
                Log this session →
              </button>
            </div>
          )}
            ...
            {[
              ...
              <div key={i} style={styles.statCard}>
                ...
                <p style={styles.statValue}>
                  {s.isText ? <span style={{ fontSize: "1.1rem" }}>{s.value}</span> : <>{s.value}<span style={styles.statUnit}>{s.unit}</span></>}
                </p>
          ...
          <div style={styles.contentGrid}>
            ...
            <div style={styles.sectionCard}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", gap: "0.5rem" }}>
                {program.weekStructure.map((d, i) => {
                  ...
                  return <WeekDot key={d.day} {...d} isToday={isToday} isDone={!!sessionOnDay && sessionOnDay.type !== "flare"} isFlare={sessionOnDay?.type === "flare"}/>;
                })}
              </div>
                  ...
                  return (
                    <div key={d.day} style={{ display: "flex", alignItems: "center", gap: "0.75rem", padding: "0.6rem 0.75rem", borderRadius: "0.65rem", background: isToday ? AMBER_LIGHT : SAGE_LIGHT }}>
                      ...
                      {isToday && !sessionOnDay && (
                        <button onClick={openTodaySession} style={{ fontSize: "0.72rem", background: AMBER_DARK, color: "#fff", border: "none", borderRadius: "100px", padding: "0.2rem 0.75rem", cursor: "pointer", fontFamily: "inherit", fontWeight: 600 }}>
                          Log
                        </button>
                      )}
                    </div>
              ...
              <div style={{ display: "flex", gap: "1rem", paddingTop: "0.5rem", borderTop: `1px solid ${CREAM}` }}>
                {[{ color: SAGE_DARK, label: "Standard" }, { color: AMBER, label: "Flare" }, { color: CREAM, border: "1px solid rgba(0,0,0,0.1)", label: "Rest" }].map(l => (
                  <div key={l.label} style={{ display: "flex", alignItems: "center", gap: "0.4rem" }}>
                    <div style={{ width: 10, height: 10, borderRadius: "50%", background: l.color, border: l.border }}/>
            ...
            {/* Quick actions + progression note */}
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "0.65rem" }}>
                  {QUICK_ACTIONS.map((a, i) => (
                    <button key={i} onClick={a.action}
                      style={{ ...styles.quickActionCard, borderLeft: `3px solid ${a.color}`, textAlign: "left" }}>
          ...
          <div ref={sessionsRef} style={styles.sectionCard}>
            <div style={styles.sectionCardHeader}>
              ...
              <button onClick={() => openLogModal()} style={{ ...styles.btnPrimary, fontSize: "0.82rem", padding: "0.55rem 1.1rem" }}>
                + Log session
              </button>
            </div>
            {sessions.length === 0 ? (
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
                ...
                {sessions.length > 8 && (
                  <p style={{ textAlign: "center", fontSize: "0.82rem", color: WARM_GRAY }}>
                    <a href="/movement/progress" style={{ color: SAGE_DARK, fontWeight: 600 }}>View all {sessions.length} sessions →</a>
                  </p>
                )}
              </div>
            )}
          ...
          <div style={{ background: SAGE_LIGHT, border: `1px solid ${SAGE}`, borderRadius: "1rem", padding: "1.25rem 1.5rem", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "1rem" }}>
            ...
            <a href="/dashboard" style={{ ...styles.btnSecondary, textDecoration: "none", whiteSpace: "nowrap", fontSize: "0.85rem" }}>Open Care Compass →</a>
          </div>
      ...
      <footer style={styles.footer}>
        <p style={styles.footerText}>Movement Compass · <a href="https://joincarecompass.com" style={styles.footerLink}>joincarecompass.com</a></p>
        <p style={styles.footerDisclaimer}>Not medical advice. Always work with your healthcare team. Movement Compass is a tool, not a replacement for professional guidance.</p>
      </footer>
      ...
      {showLogModal && (
        <LogSessionModal
          ...
          onCancel={() => { setShowLogModal(false); setLogInitial({}); }}
        />
      )}
      {showCheckin && (
        <CheckInModal onSave={handleCheckinSave} onCancel={() => setShowCheckin(false)}/>
      )}
    </div>
  );
}
...
const styles = {
  ...
  navBrand:     { display: "flex", alignItems: "center", gap: "0.65rem", flexShrink: 0 },
  ...
  quickActionCard: { background: "#fff", borderRadius: "0.875rem", padding: "1rem 1.25rem", border: "1px solid rgba(0,0,0,0.06)", display: "flex", flexDirection: "column", gap: "0.3rem", cursor: "pointer", fontFamily: "inherit", transition: "box-shadow 0.15s" },
