...
/* ─── Anchor brand tokens ──────────────────────────────────────────────────── */
const SAGE         = "#7a9e87";   // Care Compass — shared bridge color
const SAGE_LIGHT   = "#e8f0eb";
const SAGE_DARK    = "#4a7058";
const NAVY         = "#1a3a5c";   // Anchor primary
const SLATE        = "#3a6ea8";   // Anchor accent
const SLATE_MID    = "#7aa8cc";   // mid tone
const MIST         = "#eaf2f8";   // light bg
const MIST_BORDER  = "#b8d4e8";   // mist border
const TERRA        = "#b05a3a";   // flare accent
  ...
  return (
    <div style={styles.modalOverlay}>
      <div style={styles.modalCard}>
        ...
        {isFlare && (
          <div style={{ background: TERRA_LIGHT, border: `1px solid ${TERRA_BORDER}`, borderRadius: "0.75rem", padding: "0.75rem 1rem", marginBottom: "1rem", fontSize: "0.82rem", color: TERRA, lineHeight: 1.6 }}>
...
function CheckInModal({ onSave, onCancel }) {
  ...
  const rec = (() => {
    ...
    if (feel === "flare" || (feel === "okay" && energy === "low")) {
      ...
      return { label: `${name} — gentle joint input`, sessionName: name, color: TERRA, bg: TERRA_LIGHT, border: TERRA_BORDER };
    }
    ...
    <div style={styles.modalOverlay}>
      <div style={{ ...styles.modalCard, maxWidth: 520 }}>
          ...
          <>
            <div style={{ marginBottom: "1.25rem" }}>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: "0.5rem" }}>
                {JOINTS.map(j => (
                  <div key={j} onClick={() => toggleJoint(j)} style={{ padding: "0.4rem 0.9rem", borderRadius: "100px", border: `1.5px solid ${joints.includes(j) ? NAVY : "rgba(0,0,0,0.1)"}`, background: joints.includes(j) ? MIST : "#fff", fontSize: "0.82rem", color: joints.includes(j) ? NAVY : WARM_GRAY, cursor: "pointer", fontWeight: joints.includes(j) ? 600 : 400, transition: "all 0.15s" }}>{j}</div>
                ))}
              </div>
            </div>
            <div style={{ marginBottom: "1.25rem" }}>
        ...
        <div style={{ display: "flex", gap: "0.75rem" }}>
          ...
          <button onClick={() => { if (feel) onSave({ date: todayStr(), feel, joints, energy, recommendation: rec?.label, sessionName: rec?.sessionName }); }} disabled={!feel}
            style={{ ...styles.btnPrimary, background: feel === "flare" ? TERRA : NAVY, opacity: !feel ? 0.4 : 1 }}>
...
export default function AnchorDashboard() {
  ...
  const streak = (() => {
    ...
    for (const d of dates) {
      if (Math.round((cursor - new Date(d)) / 86400000) > 1) break;
      count++; cursor = new Date(d);
    }
    return count;
  })();
  ...
  function openFlareSession() { openLogModal({ sessionName: "Flare Session B", type: "flare", duration: 15 }); }
  function handleLogSave(session) {
    const updated = [session, ...sessions]; setSessions(updated); saveSessions(updated);
    setShowLogModal(false); setLogInitial({});
  }
  function handleDeleteSession(id) {
    const updated = sessions.filter(s => s.id !== id); setSessions(updated); saveSessions(updated);
  }
  function handleCheckinSave(checkin) {
    setTodayCheckin(checkin); saveCheckin(checkin); setShowCheckin(false);
    if (checkin.feel !== "rest" && checkin.sessionName) openLogModal({ sessionName: checkin.sessionName, type: inferType(checkin.sessionName) });
  }
  ...
  const QUICK_ACTIONS = [
    ...
    { label: "Log a flare day",       desc: "Switch to minimum-dose mode",                                                                           color: TERRA,   action: openFlareSession },
    { label: "Exercise library",      desc: "Browse modifications by joint",                                                                          color: SLATE,   action: () => { window.location.href = "/anchor/library"; } },
      ...
      <nav style={styles.nav}>
        <div style={styles.navInner}>
          <div style={styles.navBrand}>
            <AnchorMark size={28}/>
            <div>
              <span style={styles.navProductName}>Anchor</span>
          ...
          <div style={styles.contentGrid}>
                  ...
                  return (
                    <div key={d.day} style={{ display: "flex", alignItems: "center", gap: "0.75rem", padding: "0.6rem 0.75rem", borderRadius: "0.65rem", background: isToday ? MIST : SAGE_LIGHT }}>
                      ...
                      {isToday && !sessionOnDay && (
                        <button onClick={openTodaySession} style={{ fontSize: "0.72rem", background: NAVY, color: "#fff", border: "none", borderRadius: "100px", padding: "0.2rem 0.75rem", cursor: "pointer", fontFamily: "inherit", fontWeight: 600 }}>Log</button>
                      )}
                    </div>
                  );
                })}
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
              ...
              <div style={{ ...styles.sectionCard, background: MIST, border: `1px solid ${MIST_BORDER}` }}>
                <div>
                  <p style={{ ...styles.sectionEyebrow, color: SLATE }}>Progression note</p>
                ...
                <div style={{ fontSize: "0.78rem", color: NAVY, fontWeight: 600 }}>
          ...
          <div ref={sessionsRef} style={styles.sectionCard}>
            ...
            {sessions.length === 0 ? (
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
                ...
                {sessions.length > 8 && <p style={{ textAlign: "center", fontSize: "0.82rem", color: WARM_GRAY }}><a href="/anchor/progress" style={{ color: SLATE, fontWeight: 600 }}>View all {sessions.length} sessions →</a></p>}
              </div>
            )}
          </div>
