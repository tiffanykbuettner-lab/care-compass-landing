...
/* ─── Guidance output ────────────────────────────────────────────────────── */
function GuidanceOutput({ guidance, onReset }) {
  if (!guidance) return null;

  return (
    <div style={s.guidanceWrap}>
      <div style={s.guidanceHeader}>
        <BotanicalMark size={40}/>
        <div>
          <h2 style={s.guidanceTitle}>Your Care Compass Insights</h2>
          <p style={s.guidanceSub}>Based on what you've shared, here are patterns and areas to explore with your doctor.</p>
        </div>
      </div>

      <div style={s.disclaimer}>
        <strong>Important:</strong> This is not medical advice. Care Compass helps you identify patterns and
        prepare for conversations with your healthcare providers. Always consult a qualified medical professional.
      </div>
      ...
      <div style={s.guidanceContent}>
        {guidance.split("\n").filter(l => l.trim()).map((line, i) => {
          ...
          return <p key={i} style={s.guidancePara}>{line}</p>;
        })}
      </div>

      <div style={s.guidanceFooter}>
        <p style={s.guidanceFooterNote}>
          🌿 Remember — you are your own best advocate. Bring these insights to your next appointment
          and ask your provider to help you explore them.
        </p>
        <button onClick={onReset} style={s.resetBtn}>Start a New Assessment</button>
      </div>
    </div>
  );
}
      ...
      <main style={s.main}>
        ...
        {step === 4 ? (
          <div style={s.container}>
            <GuidanceOutput guidance={guidance} onReset={handleReset}/>
          </div>
        ) : (
          <div style={s.container}>
...
const s = {
  ...
  guidanceHeader: { display: "flex", gap: "1.25rem", alignItems: "flex-start" },
  guidanceTitle: { fontFamily: "'Playfair Display', Georgia, serif", fontSize: "1.6rem", fontWeight: 700, color: INK, margin: "0 0 0.4rem", lineHeight: 1.2 },
